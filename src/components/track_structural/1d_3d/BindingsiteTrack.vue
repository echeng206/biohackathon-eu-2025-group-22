<script setup>
    ...
    const props = defineProps({
        id: { type: String, required: false, default: 'binding_site' },
    ...
    const bindingsitesEl = ref(null);  
    onMounted(() => {
        if (bindingsitesEl.value) 
          bindingsitesEl.value.data = props.bindingSites.map(d => ({
            start: d.start,
            end: d.end,
            color: d.color || "blue",
            label: d.label || "label",
